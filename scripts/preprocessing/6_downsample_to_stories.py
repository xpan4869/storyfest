# Modified Script 6: Downsample to Stories from Filtered TR-Aligned Data (Script 5)
...
for run in runs:
    ...
    for sub in SUBJ_IDS:
        ...
        for story in story_order_in_run:
            ...
            current_time += max_end + 2

        if subject_rows:
            df_sub = pd.DataFrame(subject_rows)
            df_sub['z_pupil'] = stats.zscore(df_sub['mean_pupil_size'], nan_policy='omit')
            df_sub.to_csv(os.path.join(current_save, f"{sub}_{group_num}_story_aligned.csv"), index=False)
            ...
            plt.tight_layout()
            plot_path = os.path.join(current_save, f"{sub}_{group_num}_story_plot.png")
            plt.savefig(plot_path, dpi=300)
            plt.close()
            print(f"Saved: {plot_path}")
        print(f"Sub {sub}: Loaded pupil (first 5 values): {pupil_array[:5]}")
...
# SAVE_PATH = os.path.normpath(os.path.join(_THISDIR, '../../data/pupil/3_processed/6_storylocked/' + EXP_TYPE))
# EVENTS_PATH = os.path.normpath(os.path.join(_THISDIR, '../../experiment/Storyfest_Event_Segmentation.xlsx'))
# os.makedirs(SAVE_PATH, exist_ok=True)
...
# def time_str_to_sec(t):
#     if isinstance(t, datetime.time):
#         return t.hour * 60 + t.minute # change to t.hour * 3600 + t.minute * 60 + t.second?
#     return np.nan
...
#         #     print(f"No valid stories for subject {sub}")
#         #     continue
...
#         # Save subject CSV
#         out_csv = os.path.join(current_save, f"{sub}_{group_num}_story_aligned.csv")
...
# # ---------- Combined Plot: 6 Subplots with Correct Valence Column Placement ---------- #
# valence_bins = {'negative': [], 'neutral': [], 'positive': []}
# for story, valence in STORY_VALENCE.items():
#     if story in all_story_segments:
#         valence_bins[valence].append(story)
...
# plot_positions = {
#     0: [0, 3],  # negative: left column
#     1: [1, 4],  # neutral: middle column
#     2: [2, 5],  # positive: right column
# }
...
# plt.suptitle(f"Average Pupil Time Courses by Story Sorted by Valence {FILTER_TYPE, LOWCUT_HZ, HIGHCUT_HZ}", fontsize=18)
# plt.tight_layout(rect=[0, 0, 1, 0.95])
# combined_plot_path = os.path.join(SAVE_PATH, f"all_stories_avg_time_courses_sorted_by_valence_{FILTER_TYPE, LOWCUT_HZ, HIGHCUT_HZ}.png")
# plt.savefig(combined_plot_path, dpi=300)
# plt.close()
# print(f"Saved combined 6-story plot: {combined_plot_path}")
